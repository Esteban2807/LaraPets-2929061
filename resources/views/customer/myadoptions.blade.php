    ...
    <div class="datalist flex flex-col gap-4 items-center justify-center">
        @forelse ($adoptions as $adopt)
            <div class="avatar-group mt-2 -space-x-6">
                <div class="avatar">
                    <div class="w-36">
                        <img src="{{ asset($adopt->user->photo) }}" />
                    </div>
                </div>
                <div class="avatar">
                    <div class="w-36">
                        <img src="{{ asset($adopt->pet->image) }}" />
                    </div>
                </div>
            </div>
            <h4 class="text-white">
        ...
        @empty
            <span class="border-b-1 border-dashed mt-2 border-[#fff6] h-2 w-12/12">No adoptions found</span>
        @endforelse
    </div>
...
@section('js')
    ...
    <script>
        ...
        const search = debounce(function (query) {
            ...
            $.post("search/myadoptions", { 'q': query, '_token': $token },
                function (data) {
